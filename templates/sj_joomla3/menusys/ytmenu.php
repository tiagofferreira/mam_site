...
if (!class_exists('YtMenu')){
	class YtMenu extends YtObject{
		...
		public function getContent($tpl='default'){
			...
			$overload = $this->params->get('basepath').'/class/common/html/'. $menustyle.'/'.$tpl.'.php';
			$template = $this->params->get('basepath').'/class/'.$menustyle.'/tmpl/'.$tpl.'.php';
		...
		private function _getAssetsPath(){ 
			$menustyle = strtolower($this->params->get('menustyle', 'basic'));
			$abspath = $this->params->get('basepath').'/class/'.$menustyle.'/assets/';
			...
			if(JPATH_BASE!='/'){
				...
				$relpath = str_replace(DIRECTORY_SEPARATOR, "/", $relpath . DIRECTORY_SEPARATOR);
			}else{
				$relpath = str_replace(DIRECTORY_SEPARATOR, "/", $abspath . DIRECTORY_SEPARATOR);
			}
